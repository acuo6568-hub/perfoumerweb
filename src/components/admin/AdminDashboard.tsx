          ...
          <div className={cx(panelClass, "overflow-hidden")}>
            ...
            <div className="mt-6 grid gap-5">
              <div className="relative mx-auto grid h-[148px] w-[148px] place-items-center">
                <svg viewBox="0 0 140 140" className="h-[148px] w-[148px]">
                  <circle cx="70" cy="70" r={donutRadius} fill="none" stroke="#EEF0F4" strokeWidth="14" />
                  <circle
                    cx="70"
                    cy="70"
                    r={donutRadius}
                    ...
                    strokeWidth="14"
                    strokeLinecap="round"
                    strokeDasharray={`${donutCircumference * 0.88} ${donutCircumference * 0.12}`}
                  ...
                  <circle
                    ...
                    strokeWidth="14"
                    strokeLinecap="round"
                    strokeDasharray={`${donutCircumference * 0.08} ${donutCircumference * 0.92}`}
                    strokeDashoffset={-donutCircumference * 0.1}
                    transform="rotate(-90 70 70)"
                  />
                </svg>
                <div className="absolute text-center">
                  <p className="text-[34px] font-bold leading-none text-zinc-950">{trafficSegments.length.toLocaleString()}</p>
                  <p className="mt-1 text-sm font-medium text-zinc-500">{copy.topCountryCount}</p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                {trafficSegments.map((item, index) => {
                  ...
                  return (
                    <div key={item.country} className="rounded-[14px] border border-zinc-100 bg-zinc-50/70 p-3">
                      ...
                      <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-zinc-200">
                        <div className="h-full rounded-full bg-indigo-500" style={{ width: `${Math.max(percent, 4)}%`, opacity: 1 - index * 0.12 }} />
                      </div>
                    </div>
                  );
                })}
