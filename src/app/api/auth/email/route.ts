...
export async function POST(request: Request) {
  ...
  if (!emailOtp) {
    return NextResponse.json({ error: "Failed to generate signup verification code." }, { status: 500 });
  }

  const subject = "Confirm your Perfoumer account";
  const text = `Welcome to Perfoumer! Your verification code is ${emailOtp}.\n\nEnter this code on the signup page to complete your account creation.`;
  const html = `
